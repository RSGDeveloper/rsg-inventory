...
function CloseTrunk()
    local vehicle, distance = RSGCore.Functions.GetClosestVehicle()
...
RSGCore.Functions.CreateClientCallback('rsg-inventory:client:vehicleCheck', function(cb)
    ...
    if vehicle ~= 0 and distance < 5 then
        ...
        if #(pos - trunkpos) < 1.5 then
            if GetVehicleDoorLockStatus(vehicle) < 2 then
                ...
                RSGCore.Functions.Notify(Lang:t('notify.vlocked'), 'error')
                return
            end
        end
    end
